    ...
    private void applyPerk(Player player, PerkType type, int rank) {
        switch (type.getKind()) {
            ...
            case CUSTOM, FIRE_RESISTANCE -> {
                // Estos perks no tocan atributos ni pociones de forma persistente: sus listeners
                // dedicados (DoubleJumpListener, PlayerListener#onIncinerator, PlayerListener#onFireDamage,
                // MagnetismTask, SwimSpeedListener) consultan PlayerPerkData#isActive/getRank en vivo.
            }
        }
    }
